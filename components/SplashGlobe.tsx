...
const ZoomOutCamera: React.FC = () => {
  ...
  useFrame(() => {
    if (camera.position.z < targetZ) {
      camera.position.z = Math.min(camera.position.z + speed, targetZ);
    }
  });
...
const GlobePoints: React.FC = () => {
  ...
  const startPositions = useMemo(() => {
    ...
    for (let i = 0; i < POINT_COUNT; i++) {
      ...
      const dist = RADIUS * 10 * Math.random();
      const j = i * 3;
      arr[j] = dir.x * dist;
      arr[j + 1] = dir.y * dist;
      arr[j + 2] = dir.z * dist;
  ...
  const colors = useMemo(() => {
    const arr = new Float32Array(POINT_COUNT * 3);
    const c0 = new THREE.Color('#FFFFFF'); // white
    const c1 = new THREE.Color('#BBBBBB'); // light gray
    for (let i = 0; i < POINT_COUNT; i++) {
  ...
  const livePositionsRef = useRef<Float32Array>(new Float32Array(startPositions));
  ...
  useFrame(() => {
    ...
    if (animationProgress.current < 1) {
      animationProgress.current = Math.min(animationProgress.current + 0.015, 1);
      const t = animationProgress.current;
      for (let i = 0; i < livePositionsRef.current.length; i++) {
        livePositionsRef.current[i] = THREE.MathUtils.lerp(
          startPositions[i],
          targetPositions[i],
          t
        );
